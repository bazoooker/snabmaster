...
<div class="basket-head">
									...
									<div class="item-element element-photo"></div>
									...
									<div class="item-element element-quantity">
										Количество
									</div>
									...
									<div class="item-element element-action"></div>
...
// ценики и сумма
$arItem["PRICE_FORMATED"]=str_replace(" руб.", "", $arItem["PRICE_FORMATED"]);

$a=str_replace(" ", "", $arItem["PRICE_FORMATED"]);
$sum=$arItem["QUANTITY"]*$a;

if ($arItem["DELAY"] == "N" && $arItem["CAN_BUY"] == "Y" && $_GET['delay']!='Y'):
...
?>
								...
								<div class="showcase-item">
										<div class="item-element element-photo">
											<div class="item-photo-holder">
												<?
												$url=$arElem->fields['CODE']."/";												
												$res = CIBlockSection::GetByID($arElem->fields['IBLOCK_SECTION_ID']);
												if($ar_res = $res->GetNext())
												  	$url= $ar_res['CODE']."/".$url;
												  // do{
												...
												// 	$res = CIBlockSection::GetByID($ar_res['IBLOCK_SECTION_ID']);
												// 	if($ar_res = $res->GetNext()){
												// 		if ($id!=$ar_res['IBLOCK_SECTION_ID']) {
												// 			$url=$ar_res['CODE']."/".$url;
												// 			// echo "1/";
												// 		}
												// 		$id=$ar_res['IBLOCK_SECTION_ID'];
												// 	}
												// }while ($id!=$ar_res['IBLOCK_SECTION_ID']);		
												$url="/catalog/".$url;									
												?>
												<a class="item-photo" style="background-image: url(<?=$img?>);" href="<?=$url?>"></a>
											</div>
										</div>
										<div class="item-element element-name">
										...
										<div class="item-element element-price unit-price">											
											<div class="item-price">
												<div class="item-heading-mobile"> <!-- появляется только на мобилках -->
													Цена за единицу
												</div>
												<span id="current_price_<?=$arItem["ID"]?>" class="price-value">
													<?=$arItem["PRICE_FORMATED"]?></span>
										...
										<div class="item-element element-quantity">
											...
											<div class="counter counter-small">
												<a class="counter-control counter-minus" href="#">
													<div class="control-option">–</div>
												</a>
												<a class="counter-control counter-plus" href="#">
													<div class="control-option">+</div>
												</a>
												...
												<input
														readonly
														inputs="QUANTITY_INPUT_<?=$arItem["ID"]?>"
														ids="<?=$arItem["ID"]?>"
														ratio="<?=$ratio?>"
														float="<?=$useFloatQuantityJS?>"
														rel="<?=$arProps['KOLICHESTVO_V_UPAKOVKE']['VALUE']?>"
														type="text"
														size="3"
											...
											<div class="counter-unit"><?=htmlspecialcharsbx($arItem["MEASURE_TEXT"])?></div>

										</div>

										<div class="item-element element-price total-price">
											<div class="item-price">
												<div class="item-heading-mobile"> <!-- появляется только на мобилках -->
													Итого
												</div>
												<span class="price-value prise">
													<?=$sum;?>
												</span>
												<span class="price-unit">руб.</span>
											</div>
										...
										<div class="item-element element-action">
											...
											<!-- отложить -->
										<a class="link-user link-user_secondary-color link-user_defer button-delay" href="<?=str_replace("#ID#", $arItem["ID"], $arUrls["delay"])?>"><?=GetMessage("SALE_DELAY")?></a>

										<a class="button-delete" href="<?=str_replace("#ID#", $arItem["ID"], $arUrls["delete"])?>"
	...
	<? elseif($_GET['delay']=='Y' &&  $arItem["DELAY"] == "Y"):?>
<?
...
<h3>Отложенные</h3>
								<div class="showcase-item">
										...
										<div class="item-element element-photo">
											<div class="item-photo-holder">
												<a class="item-photo" style="background-image: url(<?=$img?>);" href="#"></a>
											</div>
										</div>
										...
										<a class="button-delete" href="<?=str_replace("#ID#", $arItem["ID"], $arUrls["delete"])?>"
											...
											<!-- отложить -->
										<!-- <a class="button-delay" href="<?=str_replace("#ID#", $arItem["ID"], $arUrls["delay"])?>"><?=GetMessage("SALE_DELAY")?></a> -->
...
<!-- оформить -->
		<div class="bx_ordercart_order_pay_center">
			...
			?>
			<a href="javascript:void(0)" onclick="checkOut();" class="checkout btn btn_primary-color btn_order btn_size-big"><?=GetMessage("SALE_ORDER")?></a>
		</div>
